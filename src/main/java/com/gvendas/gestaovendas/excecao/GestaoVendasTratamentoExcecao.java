...
public class GestaoVendasTratamentoExcecao extends ResponseEntityExceptionHandler {
    ...
    private List<Erro> gerarListaDeErros(BindingResult bindingResult) {
        ...
        bindingResult.getFieldErrors().forEach(fieldError -> {
            String msgUsuario = tratarMensagemDeErrorParaUsuario(fieldError);
    ...
    private String tratarMensagemDeErrorParaUsuario(FieldError fieldError) {
        ...
        if(fieldError.getCode().equals(CONSTANT_VALIDATION_MIN)) {
            return fieldError.getDefaultMessage().concat(String.format(" deve ser maior ou igual a %s", fieldError.getArguments()[1]));
        }
